...
<div>
    ...
    <table id=teacherstable align="center">
        <caption><strong>Teacher's Information </strong></caption>
        <tr>
            <th width="50">Faculty No</th>
            <th width="150">Name</th>
            <th width="100">Subjects assigned</th>
        ...
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
            "SELECT f.fid,f.F_name,count(c.fid)  as subjects_assigned FROM faculty f,course c  where DELETED = \"N\"  and c.fid=f.fid group by c.fid ORDER BY fid ASC");
        while ($row = mysqli_fetch_assoc($q)) {
            echo "<tr><td>{$row['fid']}</td>
                    <td>{$row['F_name']}</td>   
                    <td>{$row['subjects_assigned']}</td><td>
